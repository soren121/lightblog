...
if(isset($_POST['comment_submit']))
{
	...
	if($query->fetchSingle() == '1')
	{
		if(utf_strlen($_POST['comment_name']) && utf_strlen($_POST['comment_email']) > 0)
		{
			...
			// Escape values
			$comment_name = sqlite_escape_string(utf_htmlspecialchars($_POST['comment_name']));
			$comment_email = sqlite_escape_string(utf_htmlspecialchars($_POST['comment_email']));
			$comment_website = sqlite_escape_string(utf_htmlspecialchars($_POST['comment_website']));
			$comment_date = time();
